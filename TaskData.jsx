    ...
    return (
        <div>
                        ...
                        {currentTasks.map(({ task, startDate, endDate, prirority, _id ,status}, i) => (
                            <tr key={_id}>
                                ...
                                <td>
                                        ...
                                        { (status !== "Completed" || status === "Not yet" )  &&   <button onClick={() => handleClick(_id)} className="btn btn-primary m-1">Edit</button>}
                                        {(status !== "Completed" && status !== "Not yet" || status === "in progress") &&<button onClick={() => handleDelete(_id) } className="btn btn-danger m-1" >Delete</button>}
            ...
            <div className="d-flex justify-content-around">
                <button className="btn btn-dark m-3" onClick={() => displayPrevData()}>prev</button>
                <button  className="btn btn-dark m-3 "  onClick={() => displayNextData()}>next</button>
            </div>
        </div>
     );
}
